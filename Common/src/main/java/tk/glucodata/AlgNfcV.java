...
public class AlgNfcV {
    ...
    private static final long nfcReadTimeout = 8000; // [ms] Increased to improve reliability on slower devices
    ...
    public static boolean enableStreaming(final Tag tag, byte[] info) {
    ...
    */
  public static boolean activate(final Tag tag, byte[] info, byte[] uid) {
        log("start activate");
...
static private byte[] nfccmdtimes(final Tag tag, final byte[] cmd,int times) {
   ...
   public static byte[] nfcinfotimes(final Tag tag,int times) {
    ...
    public static byte[] readNfcTag(Tag tag, byte[] uid, byte[] info) {
        log("readNfcTag");
        try {
